...
function snippetsToSource(snippets: LegacySnippets): string {
  ...
  // `detectModernBrowserCode` is emitted verbatim (not rebuilt) so the injected
  // script stays byte-identical to what `cspHashesFor` hashes — v8's variant
  // carries an `import.meta.resolve` prefix that a rebuild would drop.
  const detectModernBrowserCode = snippets.detectModernBrowserCode.replace(
    'import.meta.url',
    joinTrick,
  )
  return [
    ...
    `export const detectModernBrowserCode = \`${detectModernBrowserCode}\`;`,
