...
const detectModernBrowserDetector = `${['import', 'meta', 'url'].join('.')};import("_").catch(()=>1);(async function*(){})().next()`
...
export const dynamicFallbackInlineCode = `!function(){if(window.${detectModernBrowserVarName})return;console.warn("[@teages/nuxt-legacy]: loading legacy chunks, syntax error above and the same error below should be ignored");var e=document.getElementById("${legacyPolyfillId}"),n=document.createElement("script");n.src=e.src,n.onload=function(){${systemJSInlineCode}},document.body.appendChild(n)}();`

export const modernChunkLegacyGuard = `export function __nuxt_legacy_guard(){${detectModernBrowserDetector}};`
